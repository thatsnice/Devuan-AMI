  ...
  SmokeTest = class SmokeTest {
    ...
    run() {
      ...
      try {
        ...
        if (this.instanceId) {
          console.error(`  To clean up: aws ec2 terminate-instances --region ${this.region} --instance-ids ${this.instanceId}`);
        }
        console.error("");
        return false;
      }
      ...
      while (attempt < maxAttempts) {
        try {
          ...
          if (attempt >= maxAttempts) {
            throw new Error("SSH not available after 5 minutes");
          }
          execSync("sleep 5");
        }
      }
    ...
    waitForCloudInit() {
      ...
      try {
        ...
        return console.log("    ✓ Cloud-init completed");
      } catch (error1) {
        error = error1;
    ...
    // ====================================================================
    ssh(command, opts = {}) {
      var ref, ref1, result, silent, sshCmd, stdio, timeout;
      silent = (ref = opts.silent) != null ? ref : false;
      timeout = (ref1 = opts.timeout) != null ? ref1 : 30_000;
